...
async def get_ndvi_monthly_trend(year: int = Query(..., description="查询年份")):
    try:
        async with engine.connect() as conn:
            # 使用 AVG() 函数对同一个月的数据取平均值，并用 GROUP BY 分组
            query = text("""
                 SELECT CAST(month AS INTEGER) as month, AVG(ndvi) as avg_ndvi 
            ...
            if not rows:
                return {"status": "no_data", "year": year, "xAxis": [], "yAxis": []}
            ...
            for row in rows:
                ...
                # 注意这里字段名变成了 avg_ndvi
                val = round(row.avg_ndvi, 4) if row.avg_ndvi is not None else None
